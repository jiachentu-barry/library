...
import com.library.demo5.dto.BorrowRecordDTO;
import com.library.demo5.dto.BorrowStatsDTO;
...
public class BorrowController {
    ...
    public ResponseEntity<?> borrowBook(@RequestBody BorrowRequest request) {
        return ResponseEntity.status(201).body(BorrowRecordDTO.from(borrowService.borrowBook(request.username(), request.bookId())));
    }
    ...
    public ResponseEntity<?> getUserBorrows(@PathVariable String username) {
        ...
        return ResponseEntity.ok(records.stream().map(BorrowRecordDTO::from).toList());
    }
    ...
    public ResponseEntity<?> adminListBorrows(@RequestHeader(value = "X-Auth-Username", required = false) String authUsername) {
        return ResponseEntity.ok(borrowService.adminListBorrows(authUsername).stream().map(BorrowRecordDTO::from).toList());
    }
    ...
    public BorrowStatsDTO borrowStats() {
        return BorrowStatsDTO.from(borrowService.borrowStats());
    }
